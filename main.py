...
'''
curveDict=dict([])
for band in ['uv','J','H','K']:
    temp=ascii.read(os.path.join('hicken','type'+type[0],'tables',band+'.dat'))
    if band =='uv':
        color='U-B'
    else:
        color='r-'+band
    curveDict[band]=snsedextend.fitColorCurve(temp,type=type[0],color=color)
print(curveDict)
sys.exit()
'''
