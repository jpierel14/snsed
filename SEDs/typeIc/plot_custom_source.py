...
ax.set_title('Type Ic SED Extrapolation, Days from Peak=%i'%days,fontsize=18)
i=1
for filename in files:
    ...
    axes[i-1].text(.45*np.max(wave),.99*np.max(source.flux(float(days),wave)),filename,fontsize=6)
...
plt.savefig("typeIc.pdf",format='pdf')
#plt.show()
sys.exit()
##########################################################################
