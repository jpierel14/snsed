...
__all__=['WavelengthGrid','TimeSeriesGrid','extend_dispersion_files',
         'extend_variance_covariance', 'extend_info_file',]
...
__MODELDIR__ = os.path.join(os.path.abspath('.'),'SALT2.INPUT')
...
def load_sncosmo_models(modeldir=__MODELDIR__,
                        salt2subdir='salt2-4', salt2irsubdir='salt2-extended',
                        lowzsubdir='lowzIa',
    ...
    salt2ir = sncosmo.models.SALT2Source(modeldir=salt2irmodeldir,
                                         name='salt2-extended')
    modeldict['salt2-extended'] = salt2ir
...
def plot_template0_data(modeldict=None, phase=0, x1=0, c=0):
    ...
    salt2irmod = modeldict['salt2-extended']
...
def check_for_salt2_fits(sedfiledir='lowzIa',
    ...
    salt2fitfile = os.path.join(__MODELDIR__, fitresfilename)
...
def load_metadata(metadatafilename='lowz_metadata.txt'):
    """read in the low-z SN metadata from the file provided by Arturo"""
    metadatafile = os.path.join(__MODELDIR__, metadatafilename)
...
def mk_salt2ir_template0_plots(modeldir=__MODELDIR__):
    ...
    salt2irsource = sncosmo.models.SALT2Source(modeldir=salt2irmodeldir, name='salt2-extended')
    ...
    for phase  in [-5,0,15,30]:
        ...
        pl.plot( wir, fir, 'r-', label='salt2-extended')
