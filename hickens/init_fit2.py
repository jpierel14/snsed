...
for f in files:
	...
	f=f[:-8]
	if len(lc[lc['Band']=='U'])==0 and len(lc[lc['Band']=='u'])==0 and len(lc[lc['Band']=='J'])==0:
		#os.remove(f)
		continue
	...
	if f=='lc_2007aa.dat':
		...
		bounds={'t0':(np.min(lc['MJD'])-20,np.min(lc['MJD'])),'hostebv':(-1,1)}
	elif f=='lc_2010bq.dat':
		bounds={'t0':(np.min(lc['MJD']),np.min(lc['MJD'])+30),'hostebv':(-1,1)}
	else:
	'''
	bounds={'t0':(peaks[f[:-4]]-5,peaks[f[:-4]]+5),'hostebv':(-1,1)}
