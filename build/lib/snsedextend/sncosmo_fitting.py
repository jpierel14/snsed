...
def generate_chisq(data, model, signature='iminuit', invcov=None,modelcov=False):
    ...
    if invcov is not None:
        ...
        if modelcov:
            ...
            cov += mcov
        invcov2 =np.linalg.pinv(cov)
        print(invcov2.shape,invcov.shape,np.max(invcov2-invcov))
...
def fit_lc(data, model, vparam_names, bounds=None, method='minuit',
           ...
           phase_range=None, wave_range=None, warn=True,invcov=None):
